...
UrlFilter::UrlFilter(HisenseOpera& opera)
    : m_opera(opera)
{
    BROWSER_LOG(INFO, "");
}

void UrlFilter::start()
{
    m_opera.AddEventHandler(&UrlFilter::OnBeforeURLRequest, this);
    ...
    redirect_.push_back(std::make_pair("https://www.msn.com", "http://opera.com"));
}
...
void UrlFilter::OnBeforeURLRequest(opera::evt::BeforeURLRequest ime_msg) {
    BROWSER_LOG( INFO, "original_url : %s", ime_msg.original_url.c_str() );
